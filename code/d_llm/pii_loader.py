...
class PIIDataCollator(DataCollatorForTokenClassification):
    ...
    def __call__(self, features):
        ...
        # requires only input_ids and attention_mask
        ...
        if labels is not None:
            for idx, ex_labels in enumerate(labels): # padding for labels
        ...
        for key in tensor_keys: # why no error for document using torch.int64?
...
class PIIDataCollatorTrain(DataCollatorForTokenClassification):
    ...
    # Where was this used? NOT USED IN THIS SCRIPT. THEN WHY CHANGE?
    # 'cfg' becomes an attribute of 'data_collector_train' accessible as 'data_collector_train.cfg'
...
def show_batch(
    ...
    for idx in range(n_examples):
        ...
        # printing (tokens, labels) if it's PII
