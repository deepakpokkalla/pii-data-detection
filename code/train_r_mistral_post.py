...
try:
    from r_mistral.pii_dataset_post import PIIDataset
    from r_mistral.pii_loader import PIIDataCollator, show_batch
    from r_mistral.pii_model import MistralForPII
    from r_mistral.pii_optimizer import get_optimizer
...
@hydra.main(version_base=None, config_path="../conf/r_mistral", config_name="conf_r_mistral")
def run_training(cfg):
    ...
    train_ds = train_ds.filter(lambda x: len(x["span_head_idxs"]) > 0)  # at least 1 span
    ...
    accelerator.print(f"# examples in valid data after filtering: {len(valid_ds)}")
    print_line()
